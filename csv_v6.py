...
def dfAuswertung (frames, cleanPath):
    ...
    for i in range(0, len(dfs)):
        df = dfs[i]
        dfAkt = df.loc[df['day']== 'Summe Aktivität']
        gesamt = [0,0,0,0]
        for z in dfAkt.index:
            durStr = df['duration'][z]
            durArr = durStr.split(':', -1)
            gesamt = addTime(gesamt, durArr)
        sec = int(gesamt[0])*24*60*60
        sec += int(gesamt[1])*60*60
        sec += int(gesamt[2])*60
        sec += int(gesamt[3])
        for h in df.index:
            dauer = df['duration'][h]
            ...
            dauerSec = int(dauer[0])*24*60*60
            dauerSec += int(dauer[1])*60*60
            dauerSec += int(dauer[2])*60
            dauerSec += int(dauer[3])
            prozt = (dauerSec/sec)*100
            prozt = str(prozt) + '%'
            df['note'][h] = prozt
        dfs[i] = df
